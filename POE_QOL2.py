...
class MyApplication(pygubu.TkApplication):
    ...
    def __init__(self, master=None):
        ...
        DEBUG = (self.config['Config']['debug'].lower() == 'true' or 'y' in self.config['Config']
                 ['debug'].lower() or self.config['Config']['debug'].lower() != 0)
        if DEBUG:
            self.DEBUG = True
    ...
    def setup_app(self):
        ...
        self.screen_res = [
            int(dim) for dim in self.config['Config']['screen_res'].split('x')]
        if len(self.screen_res) != 2:
            raise ValueError(
                "Screen Resolution was not given correctly. Use no spaces and only a single 'x'.")
        ...
        self.item_details = dict(
            Rings=[1, 1, '#33bbee', '4', int(
                self.config['Config']['threshold'])*2],
            OneHandWeapons=[1, 3, '#bbbbbb', '1', int(
                self.config['Config']['threshold'])*2],
            BodyArmours=[2, 3, '#ee3377', '1', int(
                self.config['Config']['threshold'])],
            Helmets=[2, 2, '#cc3311', '2', int(
                self.config['Config']['threshold'])],
            Gloves=[2, 2, '#ee7733', '2', int(
                self.config['Config']['threshold'])],
            Boots=[2, 2, '#009988', '2', int(
                self.config['Config']['threshold'])],
            Belts=[2, 1, '#0077bb', '3', int(
                self.config['Config']['threshold'])],
            Amulets=[1, 1, '#33bbee', '4', int(
                self.config['Config']['threshold'])],
        )
        ...
        # the custom formatting for items that match the chaos recipe
    ...
    def remove_highlights(self, update_local_record=True):
        ...
        if self.highlighted_items:  # test that highlight actually exist that need deletion
            ...
            if update_local_record:  # update the snapshot and local record if requested
                self.unident, self.ident = self.stash_finder()
                self.latest_stash = list(
                    (self.unident.copy(), self.ident.copy()))
            return True
        else:
            return False
        ...
        # if we have sets, go into the highlighting logic
            ...
            for x in unident:
                ...
                for i in range(len(unident[x])):
                    ...
                    if self.DEBUG:
                        self.pp.pprint(
                            ('Screen Coordinates:', (cord_x, cord_y)))
                    ...
                    exec(
                        f'self.{x + str(i)}.config(background="{self.item_details[x][2]}")')
                    ...
                    self.box.bind("<Button-1>", lambda command, a=self.box, b=cord_x,
                                  c=cord_y: click_item(a, b, c))  # bind click command to object
    ...
    def check_complete_set(self):
        ...
        try:
            ...
            # If we do, continue to determine how many sets we can make
                ...
                two_slot_max_sets = min((floor(len(self.latest_stash[0]["Rings"]) / 2), floor(
                    len(self.latest_stash[0]["OneHandWeapons"]) / 2)))
                # Find limiting one-slot items
                one_slot_max_sets = min([len(self.latest_stash[0][_key]) for _key in self.latest_stash[0].keys(
                ) if _key not in ["Rings", "OneHandWeapons"]])
                # Find out if we are limited by two-slot, one-slot, or the user set maximum number of highlighted sets
                max_sets = min((two_slot_max_sets, one_slot_max_sets, int(
                ...
                if max_sets:
                    # create a dictionary of empty lists to fill in and return
                    unident_sets = {_key: []
                                    for _key in self.item_details.keys()}
                    ...
                    for key in self.item_details.keys():
                        if self.DEBUG:
                            self.pp.pprint(
                                f"Item Slot name for item in {max_sets} complete sets: {key}")
                        # we need two of these, so the maximum index is twice that of the other items
                        ...
                        for i in range(max_index):
                            unident_sets[key].append(
                                self.latest_stash[0][key][i].copy())
                            if self.DEBUG:
                                self.pp.pprint(
                    ...
                    for key in self.item_details.keys():
                        ...
                        if self.DEBUG:
                            self.pp.pprint(
                                (f"self.latest_stash entry of {key}:", self.latest_stash[0][key]))
                        ...
                        self.latest_stash[0][key] = [_ for j, _ in enumerate(
                            self.latest_stash[0][key]) if j not in indices_to_delete]
                    return unident_sets
                else:
    ...
    def refresh_me(self):
        ...
        for key, value in self.unident.items():
            ...
            exec(
                f'self.builder2.get_object("{key}").configure(text="{key[:4]}: {len(value)}/{alternative}")')
        if self.DEBUG:
    ...
    def stash_finder(self):
        ...
        """
        pos_last_unid = {'BodyArmours': [],  'Helmets': [],  'OneHandWeapons': [
        ],  'Gloves': [],  'Boots': [],  'Amulets': [],  'Belts': [],  'Rings': []}
        pos_last_id = {'BodyArmours': [],  'Helmets': [],  'OneHandWeapons': [
        ],  'Gloves': [],  'Boots': [],  'Amulets': [],  'Belts': [],  'Rings': []}
        ...
        try:
            a = requests.get(stash_tab, cookies=dict(POESESSID=(
                self.config['Config']['POESESSID'])), params=payload)
        except requests.HTTPError as exception:
            Msg.showinfo(title='POE QoL',
                         message='Could not connect to pathofexile.com.')
                    ...
                    if 'BodyArmours' in x['icon']:
                        ...
                        if 'Helmets' in x['icon']:
                            ...
                            if 'OneHandWeapons' in x['icon']:
                                pos_last_id['OneHandWeapons'].append(
                                    [x['x'], x['y']])
                            else:
                                if 'Gloves' in x['icon']:
                                    pos_last_id['Gloves'].append(
                                        [x['x'], x['y']])
                                else:
                                    if 'Boots' in x['icon']:
                                        pos_last_id['Boots'].append(
                                            [x['x'], x['y']])
                                    else:
                                        if 'Amulets' in x['icon']:
                                            pos_last_id['Amulets'].append(
                                                [x['x'], x['y']])
                                        else:
                                            if 'Belts' in x['icon']:
                                                pos_last_id['Belts'].append(
                                                    [x['x'], x['y']])
                                            else:
                                                if 'Rings' in x['icon']:
                                                    pos_last_id['Rings'].append(
                                                        [x['x'], x['y']])
        else:
    ...
    def read_default_chaos_filter_sections(self):
        ...
        with open(self.config['Config']['chaos_items_filter'], 'r') as fil:
            # read whole file into memory. each line is stored as a string in a list
            ...
            for i, line in enumerate(chaos_filter):  # loop through the lines
                ...
                if self.DEBUG:
                    ...
                    self.pp.pprint(
                        ("Result of bool test for empty line:", not _line))
                    if _line:
                        self.pp.pprint(
                            ("Result of bool test for comment:", not _line[0] == "#"))
                if not _line or not _line[0] == "#":
                    continue
                # I shouldn't need to, but I double check that the line is a comment anyway
                elif _line and _line[0] == "#":
            ...
            section_ends = [i for i in section_starts[1:]] + \
                [len(chaos_filter)+1]
            ...
            for i, j in zip(section_starts, section_ends):
                ...
                for k in range(i, j-1):  # loop through all the lines in the section
                    ...
                    # convert to strings wihtout quotes...?
                    linelistcopy = [str(_).rstrip().replace("'", '')
                                    for _ in linelistcopy]
                    # convert to strings wihtout quotes...?
                    linelistcopy = [_.replace('"', '') for _ in linelistcopy]
    ...
    def pre_process_item_filter(self):
        ...
        # Get the user home directory to look for the POE filters directory
        user_path = Path.home()
        # if the path given is absolute, lets try it as is, else let stry to figure some more information about the location
        if os.path.isabs(self.config['Config']['filter']):
            ...
            # handle if the user put in a full path or other extra information
            path_pre = os.path.join(
                user_path, "Documents", "My Games", "Path of Exile")
            path_to_filter = os.path.join(
                path_pre, self.config['Config']['filter'])
            self.main_filter_path = path_to_filter
        if self.DEBUG:
            self.pp.pprint(f"Path Prefix: {path_pre}\n")
            self.pp.pprint(
                f"Searching for filter:\n {self.main_filter_path}\n")
        ...
        if filter_exists:
            ...
            Msg.showinfo(
                'POE QoL', f'POE QoL could not automatically find a filter at {self.main_filter_path}.\n\nPlease select the folder where your pathofexile fitlers are located to create a new filter named "POEQOL_Base.filter" based on the filter of the same name in this directory.')
            self.main_filter_path = os.path.normpath(os.path.join(
                filedialog.askdirectory(), 'POEQOL_Base.filter'))
            with open('POEQOL_Base.filter', 'r') as fil:
                if self.DEBUG:
                    self.pp.pprint(
                        f"Could not find the filter as indicated in config file at: {def_filter_path}")
                    self.pp.pprint("User has selected {} to write the default filter:".format(
                        self.main_filter_path))
        ...
        # start a line counter to find the section in the filter where we should insert the dynamic text from the chaos_items_filter file (see read_default_chaos_filter_sections())
        ...
        for i, line in enumerate(self.main_filter):
            ...
            if line[0] != "#":  # If the line isn't a comment, we can just move on
                ...
                if self.DEBUG:
                    self.pp.pprint(
                        f"End of chaos recipe section found at line {self.chaos_items_sections_end_line}")
                break
                ...
                if self.DEBUG:
                    self.pp.pprint(
                        "The entire filter file was looped through. This should not happen.")
    ...
    def update_filter(self):
            ...
            try:
                ...
                if slot in self.config['Config']['ignore_threshold'] or len(self.latest_stash[0][slot]) < details[4]:
                    ...
                    if self.DEBUG:
                        self.pp.pprint(
                            f"Found {len(self.latest_stash[0][slot])} items and ignore_threshold is set to {self.config['Config']['ignore_threshold']}. The filter will now show items of {slot} slot.")
            # Try to catch some errors. Not sure if this will work, don't have time to test the string formatting and message box
            except (AttributeError, ValueError):
                # TODO: Test this error message
                Msg.showinfo(
                    title='POE QoL', message=f'Check default filter formatting. There should be a valid entry for each item slot. The last word in each line should be one of the following: {[str(_[0]) for _ in self.item_details]}')
        # flatten the list of lists for the lines that should be added to the filter file
        new_filter_lines = [
            l for slt in self.chaos_items_filter_sections.values() for l in slt]
        if self.DEBUG:
            self.pp.pprint(
        ...
        if self.DEBUG:
            self.pp.pprint((len(self.main_filter0), len(
                self.main_filter1), len(new_filter_lines)))
