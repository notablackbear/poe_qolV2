...
class MyApplication():
    ...
    def __init__(self):
        ...
        builder.add_from_file('Gui_Button_V3.ui')
        self.mainwindow = builder.get_object('Main_Frame')

        # Add the veiled-GIF-animation to the Veiled-Button
        veiled_button = self.builder.get_object('veiled')
        length_gif = 91 # number of pictures creating the gif
        updatetime_ms = 70
        frames = [PhotoImage(file='Veiled_modified.gif', format='gif -index %i' % (i)) for i in range(length_gif)]
        def update(ind):
            if ind == length_gif:
                ind = 0
            else:
                ind += 1
            frame = frames[ind - 1]
            veiled_button.configure(image=frame)
    ...
    def hightlightbox_size_calculation(self):
        ...
        if int(self.settings_data[0].split(',')[1]) == 1:  # checkbutton "windowed mode"
            ...
            self.tab_origin = self.window_props[2] + 22 / 1440 * self.window_props[1], self.window_props[
                3] + 215 / 1440 * self.window_props[1]
            self.tab_end = self.window_props[2] + 864 / 1440 * self.window_props[1], self.window_props[
                3] + 1057 / 1440 * self.window_props[1]
        else:
            self.window_mode = 'fullscreen_windowed'
            self.window_props = self.get_poe_window_location(self.window_mode)
            self.tab_origin = 22 / 1440 * self.window_props[1], 215 / 1440 * self.window_props[1]
            self.tab_end = 864 / 1440 * self.window_props[1], 1057 / 1440 * self.window_props[1]
    ...
    def update_main_filter(self):
        ignore_item_threshold_list = ['BodyArmours', 'Helmets', 'OneHandWeapons', 'Gloves', 'Boots', 'Amulets', 'Rings',
                                      'Belts']
        for i, boolval in enumerate(self.settings_data[0].split(',')[2:10]):
            if boolval != '1':
                ignore_item_threshold_list[i] = ''
        for slot, details in self.item_details.items():
            ...
            if slot in ignore_item_threshold_list or len(self.latest_stash[0][slot]) < details[4]:
    ...
    def refresh_me(self):
        ...
        for key, value in self.latest_stash[0].items():
            ...
            if key == 'Rings' or key == 'OneHandWeapons':
                ...
                colour = '#e60000'
            exec(f'self.builder2.get_object("{key}").configure(text="{itemtext_for_overlay[index]}{len(value)}", foreground="{colour}")')
            index += 1
    ...
    def stash_finder(self):
        ...
        for x in json.loads(a.text)['items']:
            ...
            if '/Scarab' in x['icon'] or '/Maps' in x['icon']:
    ...
    def overlay(self):
        if self.settings_data[0].replace(' ', '') != '':
            ...
            self.builder2.add_resource_path('.')
            self.builder2.add_from_file('Gui_Button_V3.ui')
            self.overlay_top = tk.Toplevel(self.mainwindow)
            self.Overlay_Frame = self.builder2.get_object('Overlay_Frame', self.overlay_top)
            ...
            if self.settings_data[3].replace(' ', '') != '':
                ...
                self.overlay_top.geometry('616x52+%s+%s' % (toplevel_location_data[0], toplevel_location_data[1]))
                # when old settings are loaded -> auto-fix the overlay on the desired position
                self.locked = 1
                self.lock_button = self.builder2.get_object('Lock-Button')
                self.picture = ImageTk.PhotoImage(Image.open("closed.png"))
    ...
    def lock_overlay(self):
        ...
        if self.locked:
            ...
            self.picture = ImageTk.PhotoImage(Image.open("opened.png"))
        self.lock_button.configure(image=self.picture)
